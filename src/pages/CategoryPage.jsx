...
if (found.subcategories?.length > 0) {
  ...
  let selectedSub = found.subcategories[0];

  if (subcategoryFromUrl) {

    const matchedSub = found.subcategories.find(
      (sub) =>
        sub.name.toLowerCase() ===
        decodeURIComponent(subcategoryFromUrl).toLowerCase()
    );

    if (matchedSub) {
      selectedSub = matchedSub;
    }
  }

  setActiveSubId(selectedSub.id);

  loadProducts(selectedSub.id);
          ...
          <div className="flex flex-col gap-2">
            {subcategories.map((sub) => (
              <button
                ...
                onClick={() => {
  navigate(
    `/category/${category.id}?subcategory=${encodeURIComponent(sub.name)}`
  );
